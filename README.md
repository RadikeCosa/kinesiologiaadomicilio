...
- En `/admin/patients` el loader ya no hace N+1 por paciente para `EpisodeOfCare`: usa una única lectura batch por `patientIds` vía `EpisodeOfCare?patient=Patient/{id1},Patient/{id2},...`, agrupa en memoria por paciente, resuelve `activeEpisode`/`latestEpisode` preservando comparación temporal segura para el episodio más reciente y ordena el read model por prioridad operativa con desempate estable por nombre visible.

...
describe("EncounterCreateForm", () => {
  ...
  it("renders startedAt and endedAt as required and grouped in one responsive row", () => {
    ...
    expect(html).toContain("Ayuda a dejar trazabilidad clínica de la sesión y mejora la reutilización futura para resúmenes, seguimiento e informes.");
    ...
    expect(html).toContain("Síntomas, cambios percibidos, dificultades o comentarios relevantes traídos a esta visita.");
    expect(html).toContain("Hallazgos observables de esta sesión");
    expect(html).toContain("Este campo mejora mucho la utilidad del resumen compartible.");
    expect(html).toContain("Usalo para tu interpretación clínica de la visita");
    expect(html).toContain("fatiga, dolor, síntomas o respuesta al esfuerzo");
    expect(html).toContain("mejora la utilidad del resumen compartible");
    expect(html).toContain("Plan para próxima sesión");
    expect(html).toContain("Métricas funcionales");
    expect(html).toContain("seguir evolución funcional o a comparar visitas del mismo ciclo");

...
function getEncounterStatusLabel(status: Encounter["status"]): string {
  if (status === "finished") {
        ...
        <ul className="mt-3 space-y-2">
          {encounters.map((encounter) => (
            <li key={encounter.id} className="rounded border border-slate-200 bg-white p-3 text-sm text-slate-800">
              ...
              <p className="mt-1 text-xs uppercase tracking-wide text-slate-600">Estado: {getEncounterStatusLabel(encounter.status)}</p>
            </li>
          ))}
        </ul>
      )}

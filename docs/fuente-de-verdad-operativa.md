...
- Hardening operativo vigente en `/admin`: si la lectura inicial hacia FHIR falla por indisponibilidad de red/timeout/5xx o configuración faltante de `FHIR_BASE_URL`, la superficie privada muestra un fallback seguro de segmento con mensaje operativo; los bugs no operacionales (por ejemplo, mappers o errores de programación) no se silencian y siguen escalando como error real.
